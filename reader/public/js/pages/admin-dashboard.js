    ...
    async render(params) {
      ...
      try {
        ...
        page.innerHTML = '<div class="c-container">' + Ui.adminNav('dashboard') +
          ...
          '<h2 class="c-control-center__title">' + Ui.icon('home', 'sm') + 'Admin Control Center</h2>' +
          '<p class="c-control-center__subtitle">จุดรวมงานดูแลระบบ: นำเข้า source, สั่งแปล, ตรวจตอน, จัดคำศัพท์ และตั้งค่า AI</p>' +
          '</div><a class="c-btn c-btn--primary" href="#admin/translate' + (firstNovel ? '/' + Ui.esc(firstNovel.slug) : '') + '" data-nav>' + Ui.icon('book', 'xs') + '<span>Translate Queue</span></a></div>' +
          ...
          '</div>' +
          '<div class="c-card__progress c-admin-cockpit__progress" aria-label="Translation progress ' + progressPct + '%"><span class="c-card__progress-bar"><span class="c-card__progress-fill ' + Ui.progressClass(progressPct) + '"></span></span><span class="c-card__progress-pct">' + progressPct + '%</span></div>' +
          '<div class="c-control-center__actions">' +
          '<a class="c-btn c-btn--secondary" href="#admin/import" data-nav>' + Ui.icon('library', 'xs') + '<span>Import Studio</span></a>' +
          '<a class="c-btn c-btn--secondary" href="#admin/novels" data-nav>' + Ui.icon('info', 'xs') + '<span>Library Manager</span></a>' +
          '<a class="c-btn c-btn--secondary" href="#admin/glossary" data-nav>' + Ui.icon('bookmarks', 'xs') + '<span>Glossary</span></a>' +
          '<a class="c-btn c-btn--ghost" href="#admin/provider" data-nav>' + Ui.icon('settings', 'xs') + '<span>AI Settings</span></a>' +
          '</div></section>' +
          ...
          Ui.icon('book', 'md') + '<div><div class="c-admin-dashboard__tile-title">Chapters</div><div class="c-admin-dashboard__tile-meta">เลือกตอน ซ่อม source และเปิด reader</div></div></a>' +
          '<a href="#admin/logs" class="c-card c-admin-dashboard__tile" data-nav>' +
          Ui.icon('search', 'md') + '<div><div class="c-admin-dashboard__tile-title">Audit Logs</div><div class="c-admin-dashboard__tile-meta">ตรวจผลลัพธ์รายตอนเมื่อจำเป็น</div></div></a>' +
          '<a href="#settings" class="c-card c-admin-dashboard__tile" data-nav>' +
          Ui.icon('settings', 'md') + '<div><div class="c-admin-dashboard__tile-title">Local Settings</div><div class="c-admin-dashboard__tile-meta">Reader theme, font, editor</div></div></a>' +
          '</div></section></div>';
      } catch (err) {
