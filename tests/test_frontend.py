...
def test_frontend_navigation():
    with sync_playwright() as p:
        ...
        # 1. Load Homepage
        css_response = page.request.get('http://localhost:4173/design-system.css')
        ...
        page.goto('http://localhost:4173/#novel/global-descent')
        page.wait_for_load_state('networkidle')
        ...
        page.goto('http://localhost:4173/#novel/global-descent/123')
        page.wait_for_load_state('networkidle')
        assert "123" in page.url
        
        # Check chapter content is loaded
        page.wait_for_selector('#reader-title')
        page.wait_for_function("document.getElementById('reader-title').textContent !== 'กำลังโหลด...'")
        ...
        app_style_height = page.evaluate("() => window.getComputedStyle(document.getElementById('app-layout')).height")
        app_style_overflow = page.evaluate("() => window.getComputedStyle(document.getElementById('app-layout')).overflow")
        ...
        sheets_info = page.evaluate("() => Array.from(document.styleSheets).map(s => ({href: s.href, title: s.title}))")
