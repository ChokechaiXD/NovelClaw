...
def parse_html_to_markdown(html_bytes):
    ...
    if not paragraphs:
        ...
        text_only = text_only.replace('&amp;', '&').replace('&lt;', '<').replace('&gt;', '>').replace('&quot;', '"').replace('&#39;', "'")
    ...
    try:
        with zipfile.ZipFile(epub_path, 'r') as z:
            ...
            try:
                ...
                if rootfile is None:
                    return {"ok": False, "error": "Invalid EPUB: META-INF/container.xml is missing <rootfile>"}
            ...
            if not spine_order:
                return {"ok": False, "error": "EPUB spine is empty (no chapters found in reading order)."}
                
            # Create output directories
            source_dir = os.path.join(novel_root, 'chapters', 'source')
            ...
            for href in spine_order:
                ...
                target_path = os.path.normpath(os.path.join(opf_dir, raw_href)).replace('\\', '/')
                ...
                if target_path not in z.namelist():
                    ...
                    if raw_href in z.namelist():
                        ...
                        matches = [name for name in z.namelist() if name.endswith(os.path.basename(raw_href))]
                        if matches:
                ...
                out_path = os.path.join(source_dir, f"{padded_num}.md")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Import EPUB chapters into NovelClaw raw source format.")
    parser.add_argument('--epub', required=True, help="Path to the source EPUB file.")
    parser.add_argument('--novel-root', required=True, help="Path to the target novel root directory.")
    parser.add_argument('--start-num', type=int, default=1, help="Starting chapter number (default 1).")
